...
def _sort(
    ...
    try:
        ...
        if device != torch.device("cpu"):
            ...
            logger.info(
                f"Using CUDA device: {torch.cuda.get_device_name()} {memory:.2f}GB"
            )
        ...
        clu, Wall, st, tF, clu_pre_merge, newtimes = cluster_spikes(
...
def detect_spikes(
    ops, device, bfile, tic0=np.nan, progress_bar=None, clear_cache=False, verbose=False
):
...
def cluster_spikes(
    ...
    return clu, Wall, st, tF, final_pre_merge, ctics
